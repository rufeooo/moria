...
WINDOW *newwin(l, c, by, bx) int l, c, bx, by;
{
  register WINDOW* tmp;
  int16 i, nl, nc;
  ...
  tmp->_y[0] =
      (int16*)malloc(sizeof(int16) * (tmp->_maxy + 1) * (tmp->_maxx + 1));
  if (tmp->_y[0] == NULL) {
    free(tmp->_y);
    free(tmp);
    return (NULL);
  ...
  tmp->_yend = tmpptr2;
  tmp->_lastch = tmpptr2;
  for (; tmpptr1 <= tmpptr2; tmpptr1++) *tmpptr1 = (' ' | TOUCHED);
  return (tmp);
} /* end of newwin */

/* delwin : delete a window */
/* Does not seem to be used by Moria */
void delwin(w) WINDOW* w;
{
  if (w == NULL) return;
  if (w->_flags & _SUBWIN)
    free(w->_y);
...
/* mvwin : move window */
/* Does not seem to be used by Moria */
int mvwin(w, y, x) register WINDOW* w;
int y, x;
{
...
/* Does not seem to be used by Moria */
WINDOW* subwin(w, l, c, by, bx) WINDOW* w;
int l, c, by, bx;
{
  WINDOW* tmp;
  int16 i, nl, nc;
  ...
  if (nl == LINES && nc == COLS)
    ...
    tmp->_flags = _SUBWIN;
  if (by < w->_begy || by >= (w->_maxy + w->_begy) || bx < w->_begx ||
      bx >= (w->_maxx + w->_begx)) {
    free(tmp);
    return (NULL);
  };
...
/* Does not seem to be used by Moria */
void longname(termbuf, name) char *termbuf, *name;
{
  strcpy(name, ttytype);
} /* end of longname */

/* curaddch : add a character to the current screen */
int curaddch(c) char c;
{
  int16 i, tpos, *tmpptr;
...
int waddch(w, c) register WINDOW* w;
char c;
{
  int16 i, tpos, *tmpptr;
      ...
      default:
        ...
        if (w->_firstch == NULL) {
          ...
          if (w->_firstch > tmpptr)
            ...
            if (w->_lastch < tmpptr) w->_lastch = tmpptr;
          };
        };
...
int waddstr(w, s) register WINDOW* w;
register char* s;
{
  if (w == curscr)
    while (*s != '\0') curaddch(*s++);
  else
    while (*s != '\0') waddch(w, *s++);
  return (OK);
} /* end of waddstr */
...
void wclrtobot(w) WINDOW* w;
{
  register int16 j, **tmpptr1, *tmpptr2; /* temporary pointers for loops */

  tmpptr1 = &(w->_y[w->_cury]);
  for (j = w->_curx, tmpptr2 = tmpptr1[w->_curx]; j <= w->_maxx; j++, tmpptr2++)
    *tmpptr2 = ' ' | TOUCHED;
  tmpptr1++;
  ...
  if (w == curscr) {
    ...
    if (w->_firstch == NULL) {
      ...
      if (w->_firstch > tmpptr2) w->_firstch = tmpptr2;
      w->_lastch = w->_yend;
    };
  };
} /* end of wclrtobot */
...
void wclrtoeol(w) WINDOW* w;
{
  int16 i, *tmpptr2;
  register int16* tmpptr1; /* Temporary pointer for loops */

  for (i = w->_curx, tmpptr1 = &(w->_y[w->_cury][w->_curx]); i <= w->_maxx;
       i++, tmpptr1++)
    *tmpptr1 = ' ' | TOUCHED;
  if (w == curscr) {
    ...
    if (w->_firstch == NULL) {
      ...
      if (w->_firstch > tmpptr1) w->_firstch = tmpptr1;
      if (w->_lastch < tmpptr2) w->_lastch = tmpptr2;
    };
  };
} /* end of wclrtoeo */
...
int wdelch(w) WINDOW* w;
{
  ...
  for (tmpptr1 = &(w->_y[w->_cury][w->_curx]), tmpptr2 = tmpptr1 + 1;
       tmpptr1 < tmpptr3; tmpptr1++, tmpptr2++)
  ...
  *tmpptr3 = ' ' | TOUCHED;
  ...
  if (w == curscr) {
    ...
    if (w->_firstch == NULL) {
      ...
      if (w->_firstch > tmpptr1) w->_firstch = tmpptr1;
      if (w->_lastch < tmpptr3) w->_lastch = tmpptr3;
    };
  };
  return (OK);
} /* end of wdelch */
...
/* Does not seem to be used by Moria */
int wdeleteln(w) WINDOW* w;
{
  int16 i, j;
  register int16 **tmpptr1, *tmpptr2, *tmpptr3;
  /* temporary pointers for loops */

  for (i = w->_cury + 1, tmpptr1 = &(w->_y[w->_cury]); i <= w->_maxy;
       i++, tmpptr1++)
    for (j = 0, tmpptr2 = *tmpptr1, tmpptr3 = *(tmpptr1 + 1); j <= w->_maxx;
         j++, tmpptr2++, tmpptr3++)
...
void werase(w) register WINDOW* w;
{
...
/* Does not seem to be used by Moria */
void winsertln(w) WINDOW* w;
{
  int16 i, **tmpptr1, **tmpptr2;
  register int16 j, *tmpptr3, *tmpptr4;

  for (i = w->_maxy - 1, tmpptr1 = &(w->_y[w->_maxy]),
      tmpptr2 = &(w->_y[w->_maxy - 1]);
       i >= w->_cury; --i, --tmpptr1, --tmpptr2) {
    for (j = 0, tmpptr3 = *tmpptr1, tmpptr4 = *tmpptr2; j <= w->_maxx;
         j++, tmpptr3++, tmpptr4++)
  ...
  if (w == curscr) {
    ...
    if (w->_firstch == NULL)
      ...
      if (w->_firstch > w->_y[w->_cury]) w->_firstch = w->_y[w->_cury];
    };
    w->_lastch = w->_yend;
  };
} /* end of winsertln */
...
int y, x;
{
  ...
  if (y < 0) {
    ...
    if (w->_scroll) {
      for (i = w->_maxy; i < y; i++) scroll(w);
    };
    w->_cury = w->_maxy;
    return (ERR);
  } else
    w->_cury = y;
  if (w == curscr) _movcur(y, x);
  return (OK);
} /* end of wmove */
...
/* Does not seem to be used by Moria */
void overlay(v, w) WINDOW *v, *w;
{
  int16 i, j;
  /* temporary pointers for loops */
  register int16 **tmpptr1, **tmpptr2, *tmpptr3, *tmpptr4;

  for (i = 0, tmpptr1 = v->_y, tmpptr2 = w->_y; i <= v->_maxy && i <= w->_maxy;
       i++, tmpptr1++, tmpptr2++)
    for (j = 0, tmpptr3 = *tmpptr1, tmpptr4 = *tmpptr2;
         j <= v->_maxx && j <= w->_maxx; j++, tmpptr3++, tmpptr4++)
      if ((*tmpptr3 & 0xff) != 0x20) *tmpptr4 = *tmpptr3 | TOUCHED;
  if (w == curscr)
    wrefresh(curscr);
  else {
    w->_firstch = w->_y[0];
...
void overwrite(v, w) WINDOW *v, *w;
{
  ...
  for (tmpptr1 = w->_y, tmpptr2 = v->_y;
       *tmpptr1 <= w->_yend && *tmpptr2 <= v->_yend; tmpptr1++, tmpptr2++)
    for (j = 0, tmpptr3 = *tmpptr1, tmpptr4 = *tmpptr2;
         j <= w->_maxx && j <= v->_maxx; j++, tmpptr3++, tmpptr4++)
      *tmpptr3 = *tmpptr4 | TOUCHED;
  if (w == curscr)
    wrefresh(curscr);
  else {
...
long wgetch(w) WINDOW* w;
{
  ...
  Bconout(2, 'e');
  retval = getch();
  if (_modinp == COOKED) retval &= 0x00ff;
  /* Hide cursor */
  Bconout(2, '\033');
  Bconout(2, 'f');
  if (_doecho) waddch(w, (char)(0x00ff & retval));
  return (retval);
} /* end of wgetch */
...
void wrefresh(w) register WINDOW* w;
{
  ...
  } else /* ie : w != curscr */
  {
    ...
    } else /* w->_firstch != NULL : changes have appeared */
    {
      ...
      if (linelength > COLS) linelength = COLS;
...
/* Does not seem to be used by Moria */
void mvcur(ly, lx, ny, nx) int ly, lx, ny, nx;
{
  _movcur((int16)ny, (int16)nx);
} /* end of mvcur */

/* _movcur : move cursor */
void _movcur(y, x) register int16 y, x;
{
  if (_csry == y && _csrx == x) return;
  /* Set cursor position */
  Bconout(2, '\033');
  Bconout(2, 'Y');
  Bconout(2, y + ' ');
