...
sv_write()
{
  ...
  l = 0;
  if (find_cut) l |= 0x1;
  if (find_examine) l |= 0x2;
  if (find_prself) l |= 0x4;
  if (find_bound) l |= 0x8;
  if (prompt_carry_flag) l |= 0x10;
  if (rogue_like_commands) l |= 0x20;
  if (show_weight_flag) l |= 0x40;
  if (highlight_seams) l |= 0x80;
  if (find_ignore_doors) l |= 0x100;
  if (sound_beep_flag) l |= 0x200;
  if (display_counts) l |= 0x400;
  if (death) l |= 0x80000000L; /* Sign bit */
  if (total_winner) l |= 0x40000000L;
  ...
  if (death) {
    if (ferror(fileptr) || fflush(fileptr) == EOF) return FALSE;
    return TRUE;
  }
...
#ifdef MSDOS
    if (t_ptr->tchar == wallsym) t_ptr->tchar = '#';
#endif
#ifdef ATARI_ST
    if (t_ptr->tchar == (unsigned char)240) t_ptr->tchar = '#';
#endif
    t_ptr--;
  }
#endif
...
int save_char(always_ask) int always_ask;
{
  ...
  if (rc) (void)setfileinfo(savefile, vrefnum, SAVE_FTYPE);
...
save_char()
{
  ...
  while (!_save_char(savefile)) {
    ...
    if (access(savefile, 0) < 0 ||
      ...
      if (!get_string(temp, 0, 31, 45)) return FALSE;
      if (temp[0]) (void)strcpy(savefile, temp);
    }
...
#endif
{
  ...
  *generate = TRUE;
  ...
  if (turn >= 0) msg_print("IMPOSSIBLE! Attempt to restore while still alive!");
    ...
    if ((l & 0x80000000L) == 0) {
      ...
      for (i = INVEN_WIELD; i < INVEN_ARRAY_SIZE; i++) rd_item(&inventory[i]);
      ...
      if (version_min >= 2) rd_string(died_from);
    ...
    for (i = tcptr - 1; i >= MIN_TRIX; i--) {
#ifdef MSDOS
      if (t_ptr->tchar == '#') t_ptr->tchar = wallsym;
#endif
#ifdef ATARI_ST
      if (t_ptr->tchar == '#') t_ptr->tchar = (unsigned char)240;
#endif
      t_ptr--;
    }
#endif
    ...
    *generate = FALSE; /* We have restored a cave - no need to generate. */
    ...
    error:
      ...
      if (py.misc.chp >= 0) (void)strcpy(died_from, "(alive and well)");
      character_generated = 1;
    }
...
Score will not be added to scoreboard.");
        msg_print(temp);
      } else if ((noscore & 0x04) == 0 && duplicate_character()) {
        (void)sprintf(temp,
