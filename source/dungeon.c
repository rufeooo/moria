...
dungeon()
{
  ...
  do {
    ...
    if (!(turn & 0xF) &&
      ...
      if (macgetkey(CNIL, TRUE)) disturb(0, 0);
#else
...
#endif
      check_strength();
    if (py.flags.status & PY_STUDY) prt_study();
#ifdef ATARIST_MWC
    ...
    if (MAX_MALLOC - mfptr < 10) (void)compact_monsters();
...
static char original_commands(com_val) char com_val;
{
  ...
  switch (com_val) {
    ...
    case '$':
      break;
    case '.':
      if (get_dir(CNIL, &dir_val)) switch (dir_val) {
          case 1:
            com_val = 'B';
            break;
          case 2:
...
#ifdef MAC
        if (save_char(TRUE)) exit_game();
#else
        if (save_char()) exit_game();
#endif
      ...
      if ((py.flags.blind > 0) || no_light())
        ...
        for (;;) {
          ...
          (void)sprintf(out_val,
                        "Map sector [%d,%d], which is%s your sector. Look "
                        "which direction?",
                        p_y, p_x, tmp_str);
      ...
      if (wizard) {
        ...
        switch (com_val) {
          case CTRL('A'): /*^A = Cure all*/
            ...
            (void)res_stat(A_CHR);
            f_ptr = &py.flags;
            if (f_ptr->slow > 1) f_ptr->slow = 1;
            if (f_ptr->image > 1) f_ptr->image = 1;
            break;
          case CTRL('E'): /*^E = wizchar */
            change_character();
            erase_line(MSG_LINE, 0);
...
static void regenmana(percent) int percent;
{
  ...
  /* check for overflow */
  if (p_ptr->cmana < 0 && old_cmana > 0) p_ptr->cmana = MAX_SHORT;
...
#endif
    return FALSE;
  if (known2_p(t_ptr)) return FALSE;
  if (t_ptr->ident & ID_MAGIK) return FALSE;
  if (t_ptr->tohit > 0 || t_ptr->todam > 0 || t_ptr->toac > 0) return TRUE;
  if ((0x4000107fL & t_ptr->flags) && t_ptr->p1 > 0) return TRUE;
  if (0x07ffe980L & t_ptr->flags) return TRUE;
