...
void magic_treasure(x, level) int x, level;
{
  ...
  switch (t_ptr->tval) {
...
#ifdef ATARIST_MWC
                t_ptr->flags |=
                    (holder = TR_FREE_ACT | TR_CON | TR_DEX | TR_STR);
#else
    ...
    case TV_ARROW:
      ...
      t_ptr->number = 0;
      for (i = 0; i < 7; i++) t_ptr->number += randint(6);
    ...
    case TV_FOOD:
      ...
      if (t_ptr->subval == 90) t_ptr->level = 0;
    ...
    case TV_SCROLL1:
      ...
      if (t_ptr->subval == 67) t_ptr->level = 1;
      /* scroll of light */
    ...
    case TV_POTION1: /* potions */
      /* cure light */
      if (t_ptr->subval == 76) t_ptr->level = 0;
      break;
