...
int num_offer;
{
  ...
  clen = strlen(comment);
  orig_clen = clen;
  if (num_offer == 0) last_store_inc = 0;
  i = 0;
  do {
    prt(comment, 0, 0);
    ...
    if (!get_string(out_val, 0, clen, 40)) flag = FALSE;
    for (p = out_val; *p == ' '; p++)
      ;
    if (*p == '+' || *p == '-') increment = TRUE;
    ...
    do {
      ...
      if (purchase != 0)
        ...
        if (new_offer > cur_ask) {
          ...
          if (last_offer + last_store_inc > cur_ask) last_store_inc = 0;
  ...
  *price = 0;
  ...
  if (!flag) {
    ...
    if (max_buy > max_gold) {
      ...
      final_ask = max_gold;
      msg_print(
          "I am sorry, but I have not the money to afford such \
a fine item.");
      didnt_haggle = TRUE;
    } else {
      cur_ask = max_buy;
      ...
      if (noneedtobargain(store_num, final_ask)) {
        msg_print(
            "After a long bargaining session, you agree upon \
the price.");
        cur_ask = final_ask;
        comment = "Final offer";
        didnt_haggle = TRUE;
...
static int store_sell(store_num, cur_top) int store_num, *cur_top;
{
  ...
  for (counter = 0; counter < inven_ctr; counter++) {
    ...
    mask[counter] = flag;
    if (flag) {
      if (counter < first_item) first_item = counter;
      if (counter > last_item) last_item = counter;
    } /* end of if (flag) */
  }   /* end of for (counter) */
  if (last_item == -1)
