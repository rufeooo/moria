...
void display_scores(show_player) int show_player;
{
  ...
  if (feof(highscore_fp)) /* An empty score file. */
    ...
    msg_print(
        "Sorry. This scorefile is from a different version of \
umoria.");
    msg_print(CNIL);
#if defined(MSDOS) || defined(VMS) || defined(AMIGA) || defined(MAC)
...
highscores()
{
  ...
  if (panic_save == 1) {
    msg_print(
        "Sorry, scores for games restored from panic save files \
are not saved.");
    return;
  }
  ...
  if (feof(highscore_fp)) {
    ...
    while (!feof(highscore_fp)) {
      ...
      (void)fseek(
          highscore_fp,
          (long)ftell(highscore_fp) - sizeof(high_scores) - sizeof(char),
          L_SET);
#else
