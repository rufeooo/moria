...
static void place_up_stairs(y, x) int y, x;
{
  ...
  cave_ptr = &cave[y][x];
  if (cave_ptr->tptr != 0) (void)delete_object(y, x);
  cur_pos = popt();
  cave_ptr->tptr = cur_pos;
...
static void build_type2(yval, xval) int yval, xval;
{
  ...
  switch (randint(5)) {
    ...
    case 4: /* Maze inside.	*/
      ...
      for (i = y_height; i <= y_depth; i++)
        for (j = x_left; j <= x_right; j++)
          if (0x1 & (j + i)) cave[i][j].fval = TMP1_WALL;
    ...
    case 5: /* Four small rooms.	*/
      for (i = y_height; i <= y_depth; i++) cave[i][xval].fval = TMP1_WALL;
...
cave_gen()
{
  ...
  for (i = 0; i < row_rooms; i++)
    for (j = 0; j < col_rooms; j++) room_map[i][j] = FALSE;
...
static void build_store(store_num, y, x) int store_num, y, x;
{
  ...
  for (i = y_height; i <= y_depth; i++)
    for (j = x_left; j <= x_right; j++) cave[i][j].fval = BOUNDARY_WALL;
  tmp = randint(4);
  if (tmp < 3) {
...
town_gen()
{
  ...
  set_seed(town_seed);
  for (i = 0; i < 6; i++) rooms[i] = i;
  l = 6;
  for (i = 0; i < 2; i++)
    for (j = 0; j < 3; j++) {
      k = randint(l) - 1;
      build_store(rooms[k], i, j);
      for (m = k; m < l - 1; m++) rooms[m] = rooms[m + 1];
      l--;
    }
  ...
  if (0x1 & (turn / 5000)) { /* Night	*/
    for (i = 0; i < cur_height; i++) {
      ...
      for (j = 0; j < cur_width; j++) {
        if (c_ptr->fval != DARK_FLOOR) c_ptr->pl = TRUE;
        c_ptr++;
      }
#ifdef MAC
      SystemTask();
