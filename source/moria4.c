...
disarm_trap()
{
  ...
  if (get_dir(CNIL, &dir)) {
    ...
    if (c_ptr->cptr > 1 && c_ptr->tptr != 0 &&
      ...
      if ((py.flags.blind > 0) || (no_light())) tot = tot / 10;
      if (py.flags.confused > 0) tot = tot / 10;
      if (py.flags.image > 0) tot = tot / 10;
...
static int look_ray(y, from, to) int y, from, to;
{
  ...
  if (y == 0 && to > 1 || y == x && from < GRADF * 2)
    ...
    gl_noquery = FALSE;
  if (look_see(x, y, &transparent)) return TRUE;
  if (y == x) gl_noquery = FALSE;
  ...
  init_transparent:
    ...
    do {
      if (x == max_x) /* The window is trimmed by an earlier limit. */
...
inven_type* t_ptr;
{
  ...
  if (randint(10) > 1) {
    do {
      if (in_bounds(i, j)) {
        c_ptr = &cave[i][j];
        if (c_ptr->fval <= MAX_OPEN_SPACE && c_ptr->tptr == 0) flag = TRUE;
      }
      if (!flag) {
        i = y + randint(3) - 2;
        j = x + randint(3) - 2;
