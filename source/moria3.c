  ...
  if (i > 0) {
    ...
    if (result && magic_spell[py.misc.pclass - 1][*sn].smana > py.misc.cmana) {
      if (class[py.misc.pclass].spell == MAGE)
        result = get_check(
            "You summon your limited strength to cast \
this one! Confirm?");
      else
        result = get_check(
            "The gods may think you presumptuous for \
this! Confirm?");
    }
  }
  return (result);
...
register int32u flags;
{
  ...
  number = 0;
  holder = CM_60_RANDOM;
  if ((flags & holder) && (randint(100) < 60)) number++;
  holder = CM_90_RANDOM;
  if ((flags & holder) && (randint(100) < 90)) number++;
  holder = CM_1D2_OBJ;
  if (flags & holder) number += randint(2);
  holder = CM_2D2_OBJ;
  if (flags & holder) number += damroll(2, 2);
  holder = CM_4D2_OBJ;
  if (flags & holder) number += damroll(4, 2);
  if (number > 0)
    dump = summon_object(y, x, number, i);
  else
    dump = 0;
  ...
  if (dump) {
    ...
    {
      holder = CM_CARRY_OBJ;
      if (i & 0x04) holder = CM_CARRY_OBJ | CM_SMALL_OBJ;
      res |= holder;
    }
#else
    {
      res |= CM_CARRY_OBJ;
      if (i & 0x04) res |= CM_SMALL_OBJ;
    }
#endif
    if (dump >= 256)
#ifdef ATARIST_MWC
