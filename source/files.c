...
print_objects()
{
  ...
  level = 0;
  if (!get_string(tmp_str, 0, 32, 10)) return;
  level = atoi(tmp_str);
  prt("Produce how many objects?: ", 0, 0);
  nobj = 0;
  if (!get_string(tmp_str, 0, 27, 10)) return;
  nobj = atoi(tmp_str);
  small = get_check("Small objects only?");
  if ((nobj > 0) && (level > -1) && (level < 1201)) {
    if (nobj > 10000) nobj = 10000;
#ifdef MAC
...
#endif
{
  ...
  if (!access(filename1, AREAD)) {
    ...
    if (get_check(out_val)) fd = creat(filename1, 1);
  } else
  ...
  if (fd < 0 && errno == EEXIST) {
    (void)sprintf(out_val, "Replace existing file %s?", filename1);
    if (get_check(out_val)) fd = open(filename1, O_WRONLY, 0644);
  }
#endif
#endif
  if (fd >= 0) {
