...
register int factor;
{
  ...
  if (t_ptr->flags & TR_STATS) {
    for (i = 0; i < 6; i++)
      if ((1 << i) & t_ptr->flags) bst_stat(i, amount);
  }
  if (TR_SEARCH & t_ptr->flags) {
    py.misc.srh += amount;
    py.misc.fos -= amount;
  }
  if (TR_STEALTH & t_ptr->flags) py.misc.stl += amount;
  if (TR_SPEED & t_ptr->flags) change_speed(-amount);
#ifdef ATARIST_MWC
  if (((holder = TR_BLIND) & t_ptr->flags) && (factor > 0))
    py.flags.blind += 1000;
  if (((holder = TR_TIMID) & t_ptr->flags) && (factor > 0))
    py.flags.afraid += 50;
  if ((holder = TR_INFRA) & t_ptr->flags) py.flags.see_infra += amount;
#else
  if ((TR_BLIND & t_ptr->flags) && (factor > 0)) py.flags.blind += 1000;
  if ((TR_TIMID & t_ptr->flags) && (factor > 0)) py.flags.afraid += 50;
  if (TR_INFRA & t_ptr->flags) py.flags.see_infra += amount;
#endif
}
...
calc_bonuses()
{
  ...
  if (holder & item_flags) p_ptr->slow_digest = TRUE;
  holder = TR_AGGRAVATE;
  if (holder & item_flags) p_ptr->aggravate = TRUE;
  holder = TR_TELEPORT;
  if (holder & item_flags) p_ptr->teleport = TRUE;
  holder = TR_REGEN;
  if (holder & item_flags) p_ptr->regenerate = TRUE;
  holder = TR_RES_FIRE;
  if (holder & item_flags) p_ptr->fire_resist = TRUE;
  holder = TR_RES_ACID;
  if (holder & item_flags) p_ptr->acid_resist = TRUE;
  holder = TR_RES_COLD;
  if (holder & item_flags) p_ptr->cold_resist = TRUE;
  holder = TR_FREE_ACT;
  if (holder & item_flags) p_ptr->free_act = TRUE;
  holder = TR_SEE_INVIS;
  if (holder & item_flags) p_ptr->see_inv = TRUE;
  holder = TR_RES_LIGHT;
  if (holder & item_flags) p_ptr->lght_resist = TRUE;
  holder = TR_FFALL;
  if (holder & item_flags) p_ptr->ffall = TRUE;
#endif
...
static void inven_screen(new_scr) int new_scr;
{
  ...
  if (new_scr != scr_state) {
    ...
    if (line >= scr_base) {
      ...
      while (++line <= scr_base) erase_line(line, scr_left);
    }
  }
}
...
void inven_command(command) char command;
{
  ...
  do {
    if (isupper((int)command)) command = tolower((int)command);
        ...
        /* Swap screens (for drop) */
          ...
          else /* Found an item! */
          {
            if (command == 'r' || command == 't') {
              ...
              do {
                item++;
                if (inventory[item].tval != TV_NOTHING) tmp--;
              } while (tmp >= 0);
              if (isupper((int)which) && !verify(prompt, item)) item = -1;
#ifdef ATARIST_MWC
              ...
              if (item >= 0) {
                if (command == 'r') {
                  ...
                  if (inven_ctr == 0 && equip_ctr == 0) inven_weight = 0;
                } else {
    ...
    if (which == ESCAPE || scr_state == BLANK_SCR)
      ...
      if (scr_state == INVEN_SCR) {
        if (!show_weight_flag || inven_ctr == 0)
          ...
          (void)sprintf(prt1,
                        "You are carrying %d.%d pounds. Your capacity is %d.%d "
                        "pounds. %s",
                        inven_weight / 10, inven_weight % 10,
  ...
  if (scr_state != BLANK_SCR) restore_screen();
  calc_bonuses();
}
...
int* dir;
{
  ...
  if (prompt == CNIL) prompt = "Which direction?";
  for (;;) {
    ...
    command_count = save;
    if (rogue_like_commands) command = map_roguedir(command);
  ...
  for (i = start_row; i <= end_row; i++)
    for (j = start_col; j <= end_col; j++) {
      ...
      if (c_ptr->lr && !c_ptr->pl) {
        c_ptr->pl = TRUE;
        if (c_ptr->fval == DARK_FLOOR) c_ptr->fval = LIGHT_FLOOR;
...
rest()
{
  ...
  if ((rest_num == -MAX_SHORT) || (rest_num > 0) && (rest_num < MAX_SHORT)) {
    ...
    if (rest_num != 0) msg_print("Invalid rest count.");
    erase_line(MSG_LINE, 0);
    free_turn_flag = TRUE;
  }
}
