...
int16 object_offset(t_ptr) inven_type* t_ptr;
{
  switch (t_ptr->tval) {
    ...
    case TV_POTION2:
      return (5);
    case TV_FOOD:
      if ((t_ptr->subval & (ITEM_SINGLE_STACK_MIN - 1)) < MAX_MUSH) return (6);
      return (-1);
    default:
      return (-1);
  }
...
int known1_p(i_ptr) inven_type* i_ptr;
{
  ...
  if ((offset = object_offset(i_ptr)) < 0) return OD_KNOWN1;
  if (store_bought_p(i_ptr)) return OD_KNOWN1;
  offset <<= 6;
  indexx = i_ptr->subval & (ITEM_SINGLE_STACK_MIN - 1);
    ...
    if (x2 < ITEM_SINGLE_STACK_MIN || x2 >= ITEM_GROUP_MIN)
      ...
      for (i = 0; i < inven_ctr; i++) {
        ...
        if (t_ptr->tval == x1 && t_ptr->subval == x2 && i != *item &&
          ...
          inven_ctr--;
          for (j = i; j < inven_ctr; j++) inventory[j] = inventory[j + 1];
...
int pref;
{
  ...
  if (!pref) {
    ...
    if (damstr[0] != '\0') (void)strcat(tmp_val, damstr);
    if (known2_p(i_ptr)) {
    ...
    if (i_ptr->ident & (ID_MAGIK | ID_EMPTY | ID_DAMD)) {
      if (i_ptr->ident & ID_MAGIK) (void)strcat(tmp_str, "magik ");
      if (i_ptr->ident & ID_EMPTY) (void)strcat(tmp_str, "empty ");
      if (i_ptr->ident & ID_DAMD) (void)strcat(tmp_str, "damned ");
    }
