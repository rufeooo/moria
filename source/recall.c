...
int roff_recall(mon_num) int mon_num;
{
  ...
  if (wizard) {
    ...
    if (mp->r_cmove & CM_ONLY_MAGIC) mp->r_attacks[0] = MAX_UCHAR;
  }
  roffpline = 0;
  roffp = roffbuf;
#ifdef ATARIST_MWC
  ...
  if (mp->r_kills) {
    roff(" A kill of this");
    if (cp->cdefense & CD_ANIMAL) roff(" natural");
    if (cp->cdefense & CD_EVIL) roff(" evil");
    if (cp->cdefense & CD_UNDEAD) roff(" undead");
  ...
  k = 0;
  for (j = 0; j < 4; j++) /* Turbo C needs a 16 bit int for the array index.  */
    if (mp->r_attacks[(int)j]) k++;
  pu = cp->damage;
