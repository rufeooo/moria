...
#endif
    put_buffer(tmp, 23, 38);
    if (PY_SEARCH & py.flags.status) put_buffer("Search", 23, 38);
...
int inc_stat(stat) register int stat;
{
  ...
  if (tmp_stat < 118) {
    ...
    if (tmp_stat > py.stats.max_stat[stat]) py.stats.max_stat[stat] = tmp_stat;
    set_use_stat(stat);
    prt_stat(stat);
    return TRUE;
  } else
...
prt_stat_block()
{
  ...
  if ((PY_HUNGRY | PY_WEAK) & status) prt_hunger();
  if (PY_BLIND & status) prt_blind();
...
void inven_destroy(item_val) int item_val;
{
  ...
  if ((i_ptr->number > 1) && (i_ptr->subval <= ITEM_SINGLE_STACK_MAX)) {
    ...
    for (j = item_val; j < inven_ctr - 1; j++) inventory[j] = inventory[j + 1];
  ...
  for (locn = 0;; locn++) {
    ...
    /* For items which are always known1p, i.e. never have a 'color',
       insert them into the inventory in sorted order.  */
             ...
             (typ > t_ptr->tval)) {
      for (i = inven_ctr - 1; i >= locn; i--) inventory[i + 1] = inventory[i];
...
gain_spells()
{
  ...
  if (!new_spells) {
    ...
    if (py.flags.new_spells == 0) py.flags.status |= PY_STUDY;
...
calc_hitpoints()
{
  ...
  if (hitpoints < (p_ptr->lev + 1)) hitpoints = p_ptr->lev + 1;
    ...
    *insert;
{
  ...
  if (pc <= bound) {
    ...
    if (insert) (void)strcat(out_val, insert);
