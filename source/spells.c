...
int light_area(y, x) register int y, x;
{
  ...
  if (py.flags.blind < 1) msg_print("You are surrounded by a white light.");
  light = TRUE;
  if (cave[y][x].lr && (dun_level > 0)) light_room(y, x);
...
int unlight_area(y, x) int y, x;
{
    ...
    for (i = start_row; i <= end_row; i++) {
      for (j = start_col; j <= end_col; j++) {
        ...
        if (c_ptr->lr && c_ptr->fval <= MAX_CAVE_FLOOR) {
          ...
          lite_spot(i, j);
          if (!test_light(i, j)) unlight = TRUE;
        }
      }
    }
  } else
  ...
  if (unlight && py.flags.blind <= 0) msg_print("Darkness surrounds you.");
...
int aggravate_monster(dis_affect) int dis_affect;
{
  ...
  if (aggravate) msg_print("You hear a sudden stirring in the distance!");
  return (aggravate);
}
  ...
  for (i = char_row - 1; i <= char_row + 1; i++)
    for (j = char_col - 1; j <= char_col + 1; j++) {
      ...
      if (c_ptr->fval <= MAX_CAVE_FLOOR) {
        if (c_ptr->tptr != 0) (void)delete_object(i, j);
    ...
    for (j = char_col - 1; j <= char_col + 1; j++) {
      ...
      if (c_ptr->tptr != 0) {
        if (((t_list[c_ptr->tptr].tval >= TV_INVIS_TRAP) &&
          ...
          if (delete_object(i, j)) destroy = TRUE;
        } else if ((t_list[c_ptr->tptr].tval == TV_CHEST) &&
    ...
    if ((dist > OBJ_BOLT_RANGE) || c_ptr->fval >= MIN_CLOSED_SPACE)
      ...
      if (!c_ptr->pl && !c_ptr->tl) {
        ...
        if (c_ptr->fval == LIGHT_FLOOR) {
          if (panel_contains(y, x)) light_room(y, x);
        } else
          lite_spot(y, x);
      }
      ...
      if (c_ptr->cptr > 1) {
        ...
        if (CD_LIGHT & r_ptr->cdefense) {
          if (m_ptr->ml) c_recall[m_ptr->mptr].r_cdefense |= CD_LIGHT;
  ...
  do {
    ...
    if (c_ptr->tptr != 0) {
      ...
      if ((t_ptr->tval == TV_INVIS_TRAP) || (t_ptr->tval == TV_VIS_TRAP)) {
        if (delete_object(y, x)) disarm = TRUE;
        ...
        if (c_ptr->fval <= MAX_OPEN_SPACE) {
          ...
          if (c_ptr->cptr > 1) {
            ...
            if (m_ptr->hp < 0) {
              ...
              if (monptr < c_ptr->cptr) delete_monster((int)c_ptr->cptr);
...
int recharge(num) register int num;
{
  ...
  if (!find_range(TV_STAFF, TV_WAND, &i, &j))
    ...
    if (i == 1) {
      ...
      if (known2_p(i_ptr)) clear_known2(i_ptr);
      clear_empty(i_ptr);
    }
  }
  return (res);
  ...
  do {
    ...
    if ((dist > OBJ_BOLT_RANGE) || c_ptr->fval >= MIN_CLOSED_SPACE)
      ...
      if ((randint(MAX_MONS_LEVEL) < r_ptr->level) ||
        ...
        if (!(CD_NO_SLEEP & r_ptr->cdefense)) m_ptr->csleep = 0;
        (void)sprintf(out_val, "%s is unaffected.", m_name);
        msg_print(out_val);
      } else {
        if (m_ptr->confused)
    ...
    if ((c_ptr->fval >= MIN_CAVE_WALL) && (c_ptr->fval != BOUNDARY_WALL)) {
      ...
      if (t_list[c_ptr->tptr].tval == TV_RUBBLE) {
        ...
        if (randint(10) == 1) {
          ...
          if (test_light(y, x)) msg_print("You have found something!");
        }
        lite_spot(y, x);
      } else
        (void)delete_object(y, x);
  ...
  do {
    ...
    if ((dist > OBJ_BOLT_RANGE) || c_ptr->fval >= MIN_CLOSED_SPACE)
      ...
      if (randint(MAX_MONS_LEVEL) > r_ptr->level) {
        ...
        poly = place_monster(
            y, x,
            randint(m_level[MAX_MONS_LEVEL] - m_level[0]) - 1 + m_level[0],
            FALSE);
...
int build_wall(dir, y, x) int dir, y, x;
{
  ...
  do {
    ...
    if ((dist > OBJ_BOLT_RANGE) || c_ptr->fval >= MIN_CLOSED_SPACE)
      ...
      if (c_ptr->tptr != 0) (void)delete_object(y, x);
      if (c_ptr->cptr > 1) {
...
void lose_exp(amount) int32 amount;
{
  ...
  i = 0;
  while ((player_exp[i] * m_ptr->expfact / 100) <= m_ptr->exp) i++;
