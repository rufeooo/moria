...
change_character()
{
  ...
  if (get_string(tmp_str, 0, tmp_val, 3)) {
    tmp_val = atoi(tmp_str);
    if (tmp_val > -1 && (*tmp_str != '\0')) m_ptr->wt = tmp_val;
  } else
    return;
...
wizard_create()
{
  ...
  prt("Tchar  : ", 0, 0);
  if (!get_string(tmp_str, 0, 9, 1)) return;
  i_ptr->tchar = tmp_str[0];

  prt("Subval : ", 0, 0);
  if (!get_string(tmp_str, 0, 9, 5)) return;
  tmp_val = atoi(tmp_str);
  ...
  if (!get_string(tmp_str, 0, 16, 8)) return;
  ...
  if (get_check("Allocate?")) {
    ...
    if (c_ptr->tptr != 0) (void)delete_object(char_row, char_col);
