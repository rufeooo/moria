 ...
 */
static int Hash(s, mod) register char* s;
int mod;
{
  register int h = 0;
 ...
 */
static st_Entry_Pt LookupSym(st_P, name) st_Table_Pt st_P;
char* name;
{
  st_Entry_Pt entry_P;
  int h;
  ...
  for (entry_P = st_P->tab_A[h]; entry_P; entry_P = entry_P->next_P)
    if (!strncmp(entry_P->name, name, ST_MAX_SYM_LEN)) break;
 ...
 */
static st_Entry_Pt AddSym(st_P, name) st_Table_Pt st_P;
char* name;
 ...
 */
st_Table_Pt St_NewTable(name, size) char* name;
int size;
{
  st_Table_Pt st_P;
  generic_t gval;
  ...
  if (LookupSym(tnt_P, name)) return NULL;
 ...
 */
void St_DelTable(st_P) st_Table_Pt st_P;
{
  st_Entry_Pt entry_P;
  int i;
 ...
 */
char** St_ListTable(st_P) st_Table_Pt st_P;
{
  st_Entry_Pt entry_P;
  int i, j;
  static char **list_A = NULL, *chars_P = NULL;
  ...
  for (i = 0; i < st_P->entryCt; i++) list_A[i] = chars_P + ST_MAX_SYM_LEN * i;
 ...
 */
char** St_SListTable(st_P) st_Table_Pt st_P;
{
  char** list_A;

  if (!(list_A = St_ListTable(st_P))) return NULL;
 ...
 */
int St_GetSym(st_P, name, type_P, gval_P) st_Table_Pt st_P;
char* name;
int* type_P;
generic_Pt gval_P;
{
  ...
  if (!st_P) return ST_NULL_TABLE;

  if (!(entry_P = LookupSym(st_P, name))) return ST_SYM_NOT_FOUND;
 ...
 */
int St_DefSym(st_P, name, type, gval) st_Table_Pt st_P;
char* name;
int type;
generic_t gval;
{
  ...
  if (LookupSym(st_P, name)) return ST_SYM_FOUND;
 ...
 */
int St_ReplSym(st_P, name, type, gval) st_Table_Pt st_P;
char* name;
int type;
generic_t gval;
{
 ...
 */
int St_DelSym(st_P, name) st_Table_Pt st_P;
char* name;
{
  ...
  if (!entry_P) return ST_SYM_NOT_FOUND;
 ...
 *	Get a table by name
 */
st_Table_Pt St_GetTable(name) char* name;
{
  int type;
  generic_t gval;

  if (!tnt_P) return NULL;

  if (St_GetSym(tnt_P, name, &type, &gval) != ST_SYM_FOUND) return NULL;
 ...
 */
int St_TableSize(st_P) st_Table_Pt st_P;
{
  return st_P->entryCt;
}
 ...
 */
void St_DumpTable(output_F, st_P) FILE* output_F;
st_Table_Pt st_P;
{
  st_Entry_Pt entry_P;
  int bucket;
