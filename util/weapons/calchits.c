...
void attack_blows(weight, cstr, cdex, max_blows, real_blows,
                  adj_weight) int weight,
    cstr, cdex;
...
#define calc_chance(low, high, weight) \
  max(0., min((high - low), (high - low) - (high - (650. + weight)))) / 650.

/* Critical hits, Nasty way to die...			-RAK-	*/
int critical_blow(weight, plus, dam, player_level) int weight, player_level;
double plus, dam;
{
  int critical;
...
int get_stat(stat) int* stat;
{
  int tmp, c;

  if (1 != scanf(" %d", &tmp)) return (0);
...
main()
{
  ...
  for (i = 0; i < num_weapons; i++) {
    printf("Enter weight, weapon_dam, to_hit, to_dam for weapon #%d: ", i);
    if (5 != scanf(" %d %dd%d %lf %lf", &weight[i], &num_die, &die_sides,
                   &to_hit[i], &to_dam[i])) {
      putchar('\n');
      exit(1);
    }
  ...
  while (1) {
    ...
    if (cstr == 0 && cdex == 0) break;

    printf("  Weapon | Max blows | Blows | weight ratio | hp of dam |");
    printf(" w/ critical |\n");

    for (i = 0; i < num_weapons; i++) {
      attack_blows(weight[i], cstr, cdex, &max_blows, &real_blows, &adj_weight);
#if 0
      ...
      avg_dam = (int)(real_blows * weapon_dam[i]);
      critical = critical_blow(weight[i], to_hit[i], avg_dam, player_level);
      avg_dam += (int)real_blows * to_dam[i];
      critical += (int)real_blows * to_dam[i];
#endif

      printf("  %4d   | %6d    | %4d  | %8d     | %6g    |  %7g    |\n", i,
             max_blows, real_blows, adj_weight, avg_dam, critical);
    }
    printf("\n");
  }
  return (0);
