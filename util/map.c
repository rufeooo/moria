...
print_map()
{
  ...
  if (get_string(filename2, 0, strlen(tmp_str), 64)) {
    if (strlen(filename2) > 0) (void)strcpy(filename1, filename2);
    ...
    if (page_height < 10) page_height = 10;
    ...
    do {
      j = 0;
      k = i + page_height - 1;
      if (k >= cur_height) k = cur_height - 1;
      i7++;
      i8 = 0;
      do {
        l = j + page_width - 1;
        if (l >= cur_width) l = cur_width - 1;
        i8++;
        ...
        for (m = i; m <= k; m++) {
          ...
          dun_ptr = &dun_line[3];
          for (n = j; n <= l; n++) *dun_ptr++ = loc_symbol(m, n);
          *dun_ptr++ = '\n';
          *dun_ptr++ = '\0';
