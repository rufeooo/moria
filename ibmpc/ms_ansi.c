...
static int ansi_addch(), ansi_mvaddstr(), ansi_mvprintw(), ansi_move(),
    ansi_move_tgoto(), ansi_clrtobot(), ansi_clrtoeol(), ansi_mvaddch(),
    ansi_endwin(), ansi_refresh(), ansi_clear(), ansi_noop(), ansi_initscr();
int ansi_prep();
#endif
...
static int curses_move(y, x) int y, x;
{
...
static char *getent(str, tbufp, need) char *str, **tbufp;
int need;
{
  char* value;
...
initansistr()
{
  ...
  if ((term = getenv("TERM")) == NULL) term = "ibmpc-mono";
  if (tgetent(temp, term) < 1) error("Unknown terminal type: %s.", term);
  tbufp = tbuf;
...
ansi_prep(check_ansi, domoveopt, truetgoto) int check_ansi;
int domoveopt;
int truetgoto;
{
  moveopt = domoveopt;
  ...
  output_nocrmode = output_crmode = output_nonl = output_nl = output_noecho =
...
ansi_endwin()
{
  if (TI != NULL) fputs(TE, stdout);
  return OK;
}

static int ansi_addch(ch) int ch;
{
  putc(ch, stdout);
  curcol++;
  return OK;
...
static int ansi_mvaddstr(row, col, str) int row, col;
char* str;
{
...
static int ansi_mvprintw(row, col, fmt, a1, a2, a3, a4) int row, col;
char* fmt;
...
static int ansi_move(row, col) int row, col;
{
  if (moveopt && abs(currow - row) < 3 && abs(curcol - col) < 3) {
    while (row > currow) fputs(DO, stdout), currow++;
    while (row < currow) fputs(UP, stdout), currow--;
    while (col > curcol) fputs(ND, stdout), curcol++;
    while (col < curcol) fputs(LE, stdout), curcol--;
  } else
...
static int ansi_move_tgoto(row, col) int row, col;
{
  if (moveopt && abs(currow - row) < 3 && abs(curcol - col) < 3) {
    while (row > currow) fputs(DO, stdout), currow++;
    while (row < currow) fputs(UP, stdout), currow--;
...
static int ansi_mvaddch(row, col, ch) int row, col;
char ch;
{
  ansi_move(row, col);
  putchar(ch);
