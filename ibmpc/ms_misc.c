...
getlogin()
{
  ...
  if ((cp = getenv("USER")) == NULL) cp = "player";
  return cp;
}

#ifndef __TURBOC__
unsigned int sleep(secs) int secs;
{
  time_t finish_time;

  finish_time = time((long*)NULL) + secs;
  while (time((long*)NULL) < finish_time) /* nothing */;
  return 0;
}
#endif
...
void error(fmt, a1, a2, a3, a4) char* fmt;
int a1, a2, a3, a4;
{
...
void warn(fmt, a1, a2, a3, a4) char* fmt;
int a1, a2, a3, a4;
{
 ...
 */
static FILE *fopenp(name, mode, directory) char *name, *mode, directory[];
{
  ...
  while (pathp && *pathp) {
    dp = directory;
    while (*pathp && *pathp != ';') lastch = *dp++ = *pathp++;
    if (lastch != '\\' && lastch != '/' && lastch != ':') *dp++ = '\\';
    (void)strcpy(dp, name);
  ...
  while (fgets(buf, sizeof buf, fp)) {
    ...
    if (strcmpi(opt, "GRAPHICS") == 0) {
      ...
      if (cnt == 0)
        ...
        if (bp) {
          *bp++ = '\0';
          if (*bp == 'n' || *bp == 'N') saveprompt = FALSE;
        }
        if (opt[0]) (void)strcpy(moriasav, opt);
      }
...
msdos_raw()
{
  ...
  if (old_stdin & DEVICE) ioctl(STDIN, SETBITS, old_stdin | RAW);
  if (old_stdout & DEVICE) ioctl(STDOUT, SETBITS, old_stdout | RAW);
}
...
{
  if (!rawio) return;
  if (old_stdin) (void)ioctl(STDIN, SETBITS, old_stdin);
  if (old_stdout) (void)ioctl(STDOUT, SETBITS, old_stdout);
}

static unsigned int ioctl(handle, mode, setvalue) unsigned int setvalue;
{
  union REGS regs;
