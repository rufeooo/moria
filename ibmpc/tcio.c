...
int
get_string(char* in_str, int row, int column, int slen)
{
...
void
put_buffer(char* out_str, int row, int col)
{
  vtype tmp_str;
  if (col > 79) col = 79;
...
void
erase_line(int row, int col)
{
  if (row == MSG_LINE && msg_flag) msg_print(NULL);
  gotoxy(col + 1, row + 1);
  clreol();
}
...
void
move_cursor_relative(int row, int col)
{
  row -= panel_row_prt;
  col -= panel_col_prt;
  gotoxy(col + 1, row + 1);
...
void
prt(char* str_buff, int row, int col)
{
  if (row == MSG_LINE && msg_flag) msg_print(NULL);
  gotoxy(col + 1, row + 1);
  clreol();
...
void
msg_print(char* str_buff)
{
  register int old_len;
  char in_char;
  if (msg_flag) {
    old_len = strlen(old_msg[last_msg]) + 1;
    if (old_len > MSG_LEN) old_len = MSG_LEN;
...
int
get_com(char* prompt, char* command)
{
  int res;
...
screen_map()
{
  ...
  gotoxy(1, 1);
  putch(CH(TL));
  for (i = 0; i < MAX_WIDTH / RATIO; i++) putch(CH(HE));
  putch(CH(TR));
  orow = -1;
  ...
  if (mycol > 0) gotoxy(mycol + 1, myrow + 1);
  inkey();
  restore_screen();
}

void
pause_exit(int prt_line, int delay)
{
  char dummy;
