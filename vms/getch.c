...
static unsigned short int charwaiting = FALSE, crmode_status = TRUE,
                          echo_status = FALSE;
...
kbhit()
{
  ...
  */
  ...
  struct qio_return_type {
    ...
    unsigned long int l_reserved;
  } /* reserved long word */
  qio_return;

  sys$qiow(0, kb_chan, (IO$_SENSEMODE | IO$M_TYPEAHDCNT), 0, 0, 0, &qio_return,
           0, 0, 0, 0, 0);
...
void user_name(buf) char* buf;
{
  ...
  struct getjpi_itmlst_type {
    ...
    unsigned long int return_length_addr;
  } /*actual size of return data */
  getjpi_itmlst;
...
void
vms_crmode() /* Character-Return MODE */
{
  if (kb_chan == 0) opengetch();
...
void
vms_nocrmode() /* NO Character-Return MODE */
{
  if (kb_chan != 0) closegetch();
...
int
opengetch() /* does the actual assignment work */
{
...
int
closegetch() /* performs the actual deassignment work */
{
  int rv;
