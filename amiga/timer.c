...
void clean(i) int i;
{
  switch (i) {
    case 6:
      if (currentdevice->readreq) CloseDevice(currentdevice->readreq);
    case 5:
      if (currentdevice->readreq) DeleteExtIO(currentdevice->readreq);
    case 4:
      if (currentdevice->readport) DeletePort(currentdevice->readport);
    case 1:
      free(currentdevice);
  }
}
...
void sendtimer(microsec) int microsec;
{
