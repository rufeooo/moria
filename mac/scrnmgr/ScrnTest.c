...

{
  ...
  BeginScreenWait(15);
  while (TickCount() < tick) SystemTask();
  EndScreenWait();

  return;
}

static void DoAppMenu(theItem) long theItem;
...

{
  ...
  WriteScreenStringAttr(
      "This is a test",
      MakeAttr(attrColorRed, attrColorWhite) | attrUnderlined);
  WriteScreenString(" of the Screen Manager.");
  SetScreenCursor(10, 3);
  WriteScreenString("This is only a ");
  WriteScreenStringAttr(
      "test", MakeAttr(attrColorWhite, attrColorBlack) | attrItalicized);
  ...
  sprintf(msg, "Bounds: %d %d %d %d", area.top, area.left, area.bottom,
          area.right);
...

{
  ...
  InitScreenMgr(80, 25, "This is a Test", "ScrnMgrConfig", '????', '????', NULL,
                DoAppMenu, multiColor);
    ...
    if (GetScreenKeys(&keycode, &modifiers, &ascii, &h, &v)) {
      ...
      if (!(modifiers & maskModMouse)) {
        ...
        ScrollScreen(-1, 0, &scrollRect1,
                     MakeAttr(attrColorWhite, attrColorGreen));
        ...
        sprintf(hex, "%2.2X %2.2X", (int)(unsigned char)ascii,
                (int)(unsigned char)keycode);
        SetScreenString(hex, 40, 24);
        /* If the char 'D' has been typed, go into a wait.  Notice that the D
        does not appear on the screen until the wait has ended, at the
        IdleScreenMgr call. */
        if (ascii == 'D') Wait(30);
      }
