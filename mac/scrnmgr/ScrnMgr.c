...
typedef struct ScreenRec {
  ...
  ControlHandle
      hScrollHandle;           /* handle to horiz scroll control		*/
  ...
  void (*fileMenuProc)(
      long item); /* file menu handler					*/
  void (*appMenuProc)(
      long item);         /* app menu handler         */
...
static long colors[8] = {whiteColor, blackColor, redColor,     greenColor,
...
static long WNETest(envp) SysEnvRec* envp;
...
static long NoEscTest(envp) SysEnvRec* envp;
...
static void PushQueue(keycode, modifiers, ascii, h, v) char keycode;
char modifiers;
char ascii;
short h;
short v;
...
static long PopQueue(keycode, modifiers, ascii, h, v) char* keycode;
char* modifiers;
char* ascii;
short* h;
short* v;
...
static int GtoC(gh, gv, ch, cv) short gh, gv;
short *ch, *cv;
...

{
  ...
  for (i = 0, fsize = minFontSize; (i < fontSizeCount) && (fsize < maxFontSize);
       fsize++)
    if (RealFont(theScreen.txFont, fsize)) sizes[i++] = fsize;

  if (i == 0) sizes[i++] = dfltFontSize;

  theScreen.fontSizes = (short**)NewHandle(sizeof(short) * (i + 1));

  if (theScreen.fontSizes != NULL)
    for (j = 0; j < i; j++) (*theScreen.fontSizes)[j] = sizes[j];
...
static void LoadInfo(resourceFile, rfCreator, rfType) Str255 resourceFile;
...
static void UseInfo(boundsRectPtr, sizePtr, colorStdForePtr,
                    colorStdBackPtr) Rect* boundsRectPtr;
short* sizePtr;
...
static void AdjustPortBounds(bounds) Rect* bounds;

{
  ...
  if ((wid < titleMinWid) || (hgt < titleMinHgt))
    OffsetRect(bounds, (screenBits.bounds.left + titleDfltLeft) - bounds->left\,
...

{
  ...
  theScreen.charPDims.v =
      theScreen.info.ascent + theScreen.info.descent + theScreen.info.leading;
...

{
  ...
  theScreen.vScrollPRect = theScreen.hScrollPRect = theScreen.growPRect =
...
static int RepositionScreen(deltaH, deltaV) int deltaH, deltaV;

{
  ...
  if ((newH != theScreen.origin.h) || (newV != theScreen.origin.v)) {
    ...
    updtRgn = NewRgn();
    ScrollRect(&theScreen.picPRect, theScreen.origin.h - newH,
               theScreen.origin.v - newV, updtRgn);
    InvalRgn(updtRgn);
    DisposeRgn(updtRgn);
...

{
  ...
  UseInfo(&bounds, &theScreen.txSize, &theScreen.colorStdFore,
  ...
  theScreen.mouseCRect.left = theScreen.mouseCRect.top =
      theScreen.mouseCRect.right = theScreen.mouseCRect.bottom = 0;
  ...
  theScreen.window = NewWindow(NULL, &bounds, pstrTitle, false, zoomDocProc,
                               (WindowPtr)-1, true, 0);
  ...
  theScreen.hScrollHandle = NewControl(
      theScreen.window, &theScreen.hScrollPRect, "", true, theScreen.origin.h,
      theScreen.scrnXLRect.left,
      theScreen.scrnXLRect.right - theScreen.picPRect.right, scrollBarProc, 0L);
  theScreen.vScrollHandle =
      NewControl(theScreen.window, &theScreen.vScrollPRect, "", true,
                 theScreen.origin.v, theScreen.scrnXLRect.top,
...
static void ChangeScreenFontSize(num) long num;
...

{
  ...
  while (theScreen.stack != NULL) DisposeStackTop();
...
pascal void DrawDefaultBorder(theWindow, theItem) WindowPtr theWindow;
short theItem;
...
pascal void DrawGroupRect(theWindow, theItem) WindowPtr theWindow;
short theItem;
...

{
  ...
  SetDItem(theDialog, aboutDfltBorder, userItem, (Handle)DrawDefaultBorder,
           &itsRect);
  ...
  if (theScreen.versHandle) HUnlock(theScreen.versHandle);
...

{
  ...
  SetDItem(theDialog, colorsDfltBorder, userItem, (Handle)DrawDefaultBorder,
           &itsRect);
  ...
  GetDItem(theDialog, foreColors + theScreen.colorStdFore, &itsType, &fgHandle,
           &itsRect);
  SetCtlValue((ControlHandle)fgHandle, true);

  GetDItem(theDialog, backColors + theScreen.colorStdBack, &itsType, &bgHandle,
           &itsRect);
  ...
  do {
    redrawFlag = false;
    ModalDialog(nil, &itemHit);
    if ((itemHit >= foreColors) && (itemHit <= (foreColors + 7)) &&
      ...
      SetCtlValue((ControlHandle)fgHandle, true);
      redrawFlag = true;
    } else if ((itemHit >= backColors) && (itemHit <= (backColors + 7)) &&
...
static void DoAppleMenu(item) long item;

{
  ...
  switch (item) {
    ...
    default:
      GetItem(GetMHandle(appleID), item, daName);
      GetPort(&savePort);
      OpenDeskAcc(daName);
      SetPort(savePort);
      break;
...
static void DoFileMenu(item) long item;
...
static void DoEditMenu(item) long item;
...
static void DoFontChange(hndl, item, offset) MenuHandle hndl;
long item, offset;

{
  if (item != theScreen.txNum) {
    if (theScreen.txNum > 0) CheckItem(hndl, theScreen.txNum, false);
...
static void DoScreenMenu(item) long item;

{
  switch (item) {
    ...
    default:
      DoFontChange(GetMHandle(screenID), item, 3);
      break;
  }
...
static void DoFontSizeMenu(item) long item;
...
static void DoMenuItem(select) long select;

{
  ...
  switch (menuID) {
    ...
    case screenID:
      DoScreenMenu(menuItem);
      break;

    case fontSizeID:
      DoFontSizeMenu(menuItem);
      break;

    case appID1:
    case appID2:
      if (theScreen.appMenuProc != NULL) (*theScreen.appMenuProc)(menuItem);
      break;
  }
...
static void DoDrag(whichWindow, cursorLoc) WindowPtr whichWindow;
Point* cursorLoc;
...
static void HandlePreGrow(oldWidth, oldHeight, newWidth,
                          newHeight) long oldWidth,
...
static void HandlePostGrow(oldWidth, oldHeight, newWidth,
                           newHeight) long oldWidth,
    oldHeight, newWidth, newHeight;

{
  ...
  if (newWidth != oldWidth) {
    MoveControl(theScreen.vScrollHandle, theScreen.vScrollPRect.left,
                theScreen.vScrollPRect.top);
  ...
  if (newHeight != oldHeight) {
    MoveControl(theScreen.hScrollHandle, theScreen.hScrollPRect.left,
                theScreen.hScrollPRect.top);
...
static void DoGrow(whichWindow, cursorLoc) WindowPtr whichWindow;
Point* cursorLoc;
...
static void DoZoom(whichWindow, part) WindowPtr whichWindow;
long part;
...
static pascal void ActOnScroll(theControl, partCode) ControlHandle theControl;
short partCode;
...
static void DoHScroll(cursorLoc, thePart) Point* cursorLoc;
long thePart;
...
static void DoVScroll(cursorLoc, thePart) Point* cursorLoc;
long thePart;
...
static void DoContentClick(whichWindow, cursorLoc) WindowPtr whichWindow;
Point* cursorLoc;
...
static void DoMouseDown(cursorLoc) Point* cursorLoc;

{
  ...
  switch (part = FindWindow(*cursorLoc, &whichWindow)) {
    case inMenuBar:
      DoMenuItem(MenuSelect(*cursorLoc));
      break;

    case inDrag:
      DoDrag(whichWindow, cursorLoc);
      break;

    case inGrow:
      DoGrow(whichWindow, cursorLoc);
      break;

    case inSysWindow:
      SystemClick(&theScreen.event, whichWindow);
      break;
    ...
    case inZoomOut:
      if (whichWindow == theScreen.window)
        if (TrackBox(whichWindow, *cursorLoc, part)) DoZoom(whichWindow, part);
      break;
...
static void DoKeyDown(cmdFlag) int cmdFlag;

{
  ...
  if ((theScreen.event.when > theScreen.keyFlush) &&
    ...
    if (cmdFlag) {
      upper = ascii;
      if ((upper >= 'a') && (upper <= 'z')) upper = 'A' + (upper - 'a');
      if ((upper >= '@') && (upper <= '_')) ascii = upper - '@';
    }
...
static void DoUpdate(whichWindow) WindowPtr whichWindow;
...
static void DoActivate(whichWindow, activated) WindowPtr whichWindow;
Boolean activated;
...
static void InvalScreenRect(d, r) int d;
Rect* r;
...
static void InvalCursor(d) int d;
...
void XSetScreenChar(d, c, h, v) int d;
char c;
long h, v;
...
void XSetScreenBuffer(d, c, row, bounds, h, v) int d;
char* c;
long row;
Rect* bounds;
long h, v;

{
  ...
  if (SectRect(&temp, &theScreen.scrnCRect, &area)) {
    srcLoc =
        (area.top + bounds->top - v) * row + (area.left + bounds->left - h);
...
void XSetScreenString(d, s, h, v) int d;
char* s;
long h, v;
...
void XSetScreenCharAttr(d, c, a, h, v) int d;
char c;
char a;
long h, v;
...
void XSetScreenBufferAttr(d, c, a, row, bounds, h, v) int d;
char* c;
char a;
long row;
Rect* bounds;
...
void XSetScreenStringAttr(d, s, a, h, v) int d;
char* s;
char a;
long h, v;
...
void XSetScreenImage(d, c, a, row, bounds, h, v) int d;
char* c;
char* a;
long row;
Rect* bounds;
...
void XWriteScreenChar(d, c) int d;
char c;
...
void XWriteScreenBuffer(d, c, row, bounds) int d;
char* c;
long row;
Rect* bounds;
...
void XWriteScreenString(d, s) int d;
char* s;
...
void XWriteScreenCharAttr(d, c, a) int d;
char c;
char a;
...
void XWriteScreenBufferAttr(d, c, a, row, bounds) int d;
char* c;
char a;
long row;
Rect* bounds;
...
void XWriteScreenStringAttr(d, s, a) int d;
char* s;
char a;
...
void XWriteScreenImage(d, c, a, row, bounds) int d;
char* c;
char* a;
long row;
Rect* bounds;
...
void XFillScreen(d, c, a, bounds) int d;
char c;
char a;
Rect* bounds;
...
void XEraseScreen(d, bounds) int d;
Rect* bounds;
...
static void ScrollScreenPositive(area, offset, factor) Rect* area;
long offset;
short factor;
...
static void ScrollScreenNegative(area, offset, factor) Rect* area;
long offset;
short factor;
...
void XScrollScreen(d, dh, dv, bounds, attr) int d;
long dh, dv;
Rect* bounds;
char attr;
...
void GetScreenCharAttr(c, a, h, v) char* c;
char* a;
long h, v;
...
void GetScreenImage(c, a, row, bounds, h, v) char* c;
char* a;
long row;
Rect* bounds;
long h, v;
...
void XMoveScreenCursor(d, h, v) int d;
long h, v;
...
void XSetScreenCursor(d, h, v) int d;
long h, v;
...
void GetScreenCursor(h, v) long *h, *v;
{
  ...
  while (v < theScreen.scrnCDims.v) {
    if (!*right) {
      ...
      do {
        ...
        *right++ = 0;
      } while ((v < theScreen.scrnCDims.v) && (*left == inval.left) &&
               (*right == inval.right));
...
static void UpdateScreenLine(area, c, a, len) Rect* area;
char* c;
char* a;
long len;

{
  ...
  while (a < last) {
    ...
    if (theScreen.colorFlag) {
      ...
      ForeColor(
          colors[(AttrFore(attr) == attrColorBack) ? theScreen.colorStdBack
                                                   : theScreen.colorStdFore]);
      BackColor(
          colors[(AttrBack(attr) == attrColorBack) ? theScreen.colorStdBack
                                                   : theScreen.colorStdFore]);
    }
...

{
  ...
  if (!theScreen.colorFlag) BackColor(colors[theScreen.colorStdBack]);
  ...
  if (theScreen.cursorStatus) {
    ...
    if (theScreen.colorFlag)
      ...
      cursorLoc =
          theScreen.cursor.v * theScreen.scrnCDims.h + theScreen.cursor.h;
      BackColor(
          colors[(AttrFore((*theScreen.attrs)[cursorLoc]) == attrColorBack)
                     ? theScreen.colorStdBack
                     : theScreen.colorStdFore]);
    }
    EraseRect(&curs);
...

{
  ...
  do {
    ...
    if (more) switch (theScreen.event.what) {
        case mouseDown:
          DoMouseDown(&theScreen.event.where);
          break;

        case autoKey:
        case keyDown:
          if (theScreen.event.modifiers & cmdKey) {
        ...
        case updateEvt:
          DoUpdate((WindowPtr)theScreen.event.message);
          break;

        case activateEvt:
          DoActivate((WindowPtr)theScreen.event.message,
                     theScreen.event.modifiers & activeFlag);
          break;

        case osEvent:
          if ((theScreen.event.message >> 24) == suspendResumeMessage) {
...
int GetScreenKeys(keycode, modifiers, ascii, h, v) char* keycode;
char* modifiers;
char* ascii;
short* h;
short* v;
...
void EnableScreenMouse(flag) long flag;
...
void ClipScreenMouse(area) Rect* area;
...
void DefineScreenCursor(color, lines, blinkRate) long color;
long lines;
long blinkRate;
...
void SetScreenAboutProc(procPtr) void (*procPtr)();
...
void SetScreenQuitProc(procPtr, flag) void (*procPtr)();
long flag;
...

{
  ...
  SetDItem(theDialog, yesOrNoDfltBorder, userItem, (Handle)DrawDefaultBorder,
           &itsRect);
...
void ShowScreen(visible) long visible;
...
void GetScreenBounds(bounds) Rect* bounds;
...
void CenterScreenDLOG(id, hRatio, vRatio, h, v) long id;
Fixed hRatio, vRatio;
long *h, *v;
...
long DoScreenALRT(id, kind, hRatio, vRatio) long id;
long kind;
Fixed hRatio, vRatio;

{
  ...
  if (a != NULL) {
    ...
    switch (kind) {
      case akNormal:
        item = Alert((short)id, NULL);
        break;
      case akStop:
        item = StopAlert((short)id, NULL);
        break;
      case akNote:
        item = NoteAlert((short)id, NULL);
        break;
      case akCaution:
        item = CautionAlert((short)id, NULL);
        break;
    }
...
void ConfigScreenMgr(force, theType, theID, ConfigProc) long force;
ResType theType;
long theID;
...

{
  ...
  if (theData != NULL) {
    ...
    if ((theScreen.reconfigFlag) || (force) ||
...
void BeginScreenWait(rate) long rate;
...

{
  if (theScreen.stack != NULL) {
    ...
    XSetScreenImage(0, *(*theScreen.stack)->chars, *(*theScreen.stack)->attrs,
                    theScreen.scrnCDims.h, &theScreen.scrnCRect, 0, 0);
