...

{
  ...
  for (i = 0, r = restable; i < MAX_RESOURCES; i++, r++) {
    DumpRes(argv[1], r->resType, r->resID, r->resName, RES_ATTRS, r->memPtr,
            r->elemCnt, r->elemSiz, r->strProc);
  }
  ...
  DumpRes(argv[1], restartRsrc, restart_id, "Restart Vars", RES_ATTRS, p, 1,
          size, NULL);
#endif
